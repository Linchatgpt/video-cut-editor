...
export async function chooseHighlights(transcript, { apiKey = process.env.GEMINI_API_KEY, clipCount = 3, clipDuration = 15 } = {}) {
  ...
  const prompt = `你是短影音剪輯師。請從以下帶時間戳逐字稿挑選 ${clipCount} 個最有價值的片段，每段約 ${clipDuration} 秒。價值可來自清楚觀點、反直覺洞察、情緒轉折或可獨立理解的故事。只回傳 JSON 陣列，不要 Markdown：[{"id":"clip-1","title":"15字內吸睛標題","start_time":0,"end_time":${clipDuration}}]。時間必須來自逐字稿範圍，標題使用繁體中文。\n逐字稿：${JSON.stringify(transcript.segments)}`;
  try {
