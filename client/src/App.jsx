...
export default function App() {
  ...
  const analyze = async (id) => { setError(''); setAnalyzing(true); setStatus('正在分析語音與高光片段…'); try { const r = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileId: id, clipCount, clipDuration }) }); const data = await readJson(r); if (!r.ok) throw new Error(data.error || '影片分析失敗'); const next = data.clips.map((c) => ({ ...c, top_text: c.top_text || c.title, caption: c.bottom_text || c.title, aiStart: c.start_time, aiEnd: c.end_time, outputAspect: 'original', style: makeStyle(), activeLayer: 'topText', musicFile: '', musicVolume: 0.35, keepOriginalAudio: true })); setClips(next); setSelectedClipId(next[0]?.id || null); setStatus('已找到 ' + data.clips.length + ' 個候選片段'); } catch (e) { setError(e.message); setStatus('需要處理'); } finally { setAnalyzing(false); } };
  async function upload(file) { setSelectedFile(file); setVideoUrl(URL.createObjectURL(file)); setClips([]); setSelectedClipId(null); setStatus('正在上傳…'); try { const form = new FormData(); form.append('video', file); const r = await fetch('/api/upload', { method: 'POST', body: form }); const data = await readJson(r); if (!r.ok) throw new Error(data.error); setFileId(data.fileId); setDimensions(data.dimensions); await analyze(data.fileId); } catch (e) { setError(e.message); setStatus('需要處理'); } }
  ...
  const addManualClip = () => { const start = Number(manualStart); const end = Number(manualEnd); if (!Number.isFinite(start) || !Number.isFinite(end) || start < 0 || end <= start) { setError('手動片段時間無效，請確認開始與結束時間。'); return; } const clip = { id: `manual-${Date.now()}`, source: 'manual', title: `手動片段${clips.filter((item) => item.source === 'manual').length ? clips.filter((item) => item.source === 'manual').length + 1 : ''}`, top_text: '', caption: '', start_time: start, end_time: end, aiStart: start, aiEnd: end, outputAspect: 'original', style: makeStyle(), activeLayer: 'topText', musicFile: '', musicVolume: 0.35, keepOriginalAudio: true }; setClips((all) => [...all, clip]); setSelectedClipId(clip.id); setError(''); };
...
function ClipSettings({ clip, dimensions, musicFiles, update, updateLayer, updateAspect, setLayer, resetPosition, renderOne, fonts, playing, preview }) {
  const layer = clip.activeLayer || 'topText';
  const s = clip.style[layer];
  ...
  return <aside className="settings-panel">
    ...
    {clip.musicFile && <div className="music-controls settings-music-controls"><label>音樂音量 <output>{Math.round(Number(clip.musicVolume ?? 0.35) * 100)}%</output><input type="range" min="0" max="1" step="0.05" value={clip.musicVolume ?? 0.35} onChange={(e) => update(clip.id, 'musicVolume', Number(e.target.value))} /></label><label className="checkbox-field"><input type="checkbox" checked={clip.keepOriginalAudio !== false} onChange={(e) => update(clip.id, 'keepOriginalAudio', e.target.checked)} /> 保留原影片聲音</label><button type="button" className="music-preview-button" onClick={() => preview(clip)}>{playing ? '暫停試聽' : '試聽此片段'}</button><small>播放片段時同步試聽；渲染時會套用相同設定。</small></div>}
