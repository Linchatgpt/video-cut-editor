...
async function readJson(response) {
  ...
  catch { throw new Error(`後端回應格式錯誤（HTTP ${response.status}）。請確認 API 網址設定正確。`); }
}
...
const makeCropPosition = () => ({ x: 50, y: 50 });
const fonts = [
  { value: 'Noto Sans TC', label: 'Noto Sans TC' },
  { value: 'Iansui', label: '芫荽' },
  { value: 'Hannotate TC', label: '手札體' },
  { value: 'BiauKaiTC', label: '標楷體' },
  { value: 'KouzanBrushFontOTF', label: '衡山毛筆' },
...
export default function App() {
  ...
  const updateCropPosition = (id, axis, value) => setClips((all) => all.map((c) => c.id === id ? { ...c, renderedUrl: undefined, cropPosition: { ...(c.cropPosition || makeCropPosition()), [axis]: Number(value) } } : c));
  const addManualClip = () => { const start = Number(manualStart); const end = Number(manualEnd); if (!Number.isFinite(start) || !Number.isFinite(end) || start < 0 || end <= start) { setError('手動片段時間無效，請確認開始與結束時間。'); return; } const clip = { id: `manual-${Date.now()}`, source: 'manual', title: `手動片段${clips.filter((item) => item.source === 'manual').length ? clips.filter((item) => item.source === 'manual').length + 1 : ''}`, top_text: '', caption: '', start_time: start, end_time: end, aiStart: start, aiEnd: end, outputAspect: 'original', cropPosition: makeCropPosition(), style: makeStyle(), activeLayer: 'topText', musicFile: '', musicVolume: 0.35, originalVolume: 1 }; setClips((all) => [...all, clip]); setSelectedClipId(clip.id); setError(''); };
  ...
  const hasAiClips = clips.some((clip) => clip.source !== 'manual');
  ...
  return <main className="shell">
    ...
    <section className="hero-grid"><div className="hero-copy"><p className="section-kicker">01 / 上傳素材</p><h2>找到觀眾會停下來看的 15 秒。</h2><p className="hero-description">上傳一支長影片，再選擇要用 AI 分析或手動挑選片段。</p></div><div className="upload-zone"><label className="upload-select"><input type="file" accept="video/mp4,video/quicktime,video/webm,video/x-matroska" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0])} /><span className="upload-icon">↑</span><strong>{selectedFile ? selectedFile.name : '選擇長影片'}</strong><small>MP4 / MOV / WebM / MKV · 最大 2 GB</small></label>{fileId && <button className={analyzing ? 'is-loading' : ''} type="button" disabled={analyzing} onClick={() => analyze(fileId)}>{analyzing ? 'AI 分析中…' : hasAiClips ? '重新分析這支影片' : fileId === 'default' ? '開始分析預設影片' : '開始分析這支影片'}</button>}<section className="analysis-settings"><p className="section-kicker">AI 分析參數</p><div className="analysis-fields"><label>片段數量<input type="number" min="1" max="5" value={clipCount} onChange={(e) => setClipCount(Number(e.target.value))} /></label><label>每段秒數<input type="number" min="10" max="60" value={clipDuration} onChange={(e) => setClipDuration(Number(e.target.value))} /></label></div><span>不按分析也沒關係：影片上傳完成後，可直接用下方時間新增手動片段。</span></section></div></section>
    ...
    <section className="studio-workbench">
      ...
      {selectedClip ? <ClipSettings clip={selectedClip} dimensions={dimensions} musicFiles={musicFiles} update={update} updateLayer={updateLayer} updateAspect={updateAspect} updateCropPosition={updateCropPosition} setLayer={setLayer} resetPosition={resetPosition} renderOne={renderOne} fonts={fonts} playing={playingClipId === selectedClip.id} preview={preview} /> : <aside className="settings-panel settings-empty">選擇一個片段開始編輯。</aside>}
    </section>
  </main>;
}
...
function ClipControls({ clip, update, updateLayer, resetPosition, renderOne }) {
  ...
  return <section className="preview-controls"><div className="controls-heading"><div><p className="section-kicker">05 / 調整與輸出</p><h3>位置與渲染</h3></div><span>{layer === 'topText' ? '上欄文字' : '下欄文字'}</span></div><div className="position-group"><div className="control-group-title">位置 <button type="button" onClick={() => resetPosition(clip.id, layer)}>重設位置</button></div><label>X 軸 <output>{s.x}%</output><input type="range" min="5" max="95" value={s.x} onChange={(e) => updateLayer(clip.id, layer, 'x', e.target.value)} /></label><label>Y 軸 <output>{s.y}%</output><input type="range" min="5" max="95" value={s.y} onChange={(e) => updateLayer(clip.id, layer, 'y', e.target.value)} /></label></div><div className="time-editor"><span className="setting-label">片段時間</span><div><label>開始<input type="number" min="0" step="0.1" value={clip.start_time} onChange={(e) => update(clip.id, 'start_time', e.target.value)} /></label><label>結束<input type="number" min="0" step="0.1" value={clip.end_time} onChange={(e) => update(clip.id, 'end_time', e.target.value)} /></label></div><small>AI 建議 {clip.aiStart.toFixed(1)}s – {clip.aiEnd.toFixed(1)}s</small></div><div className="settings-actions"><button type="button" className="render-one" onClick={() => renderOne(clip)} disabled={clip.rendering}>{clip.rendering ? '渲染中…' : '渲染此片段 ↗'}</button></div>{clip.renderedUrl && <div className="rendered-result"><video src={clip.renderedUrl} controls preload="metadata" onPlay={(event) => document.querySelectorAll('video').forEach((video) => { if (video !== event.currentTarget) video.pause(); })} /><a href={clip.downloadUrl || clip.renderedUrl}>下載此片段 ↓</a></div>}</section>;
}
function ClipSettings({ clip, dimensions, musicFiles, update, updateLayer, updateAspect, updateCropPosition, setLayer, resetPosition, renderOne, fonts, playing, preview }) {
  const layer = clip.activeLayer || 'topText';
  const s = clip.style[layer];
  const clipLength = Math.max(0, Number(clip.end_time) - Number(clip.start_time));
  const showFrom = Math.min(clipLength, Math.max(0, Number(s.showFrom || 0)));
  const hideAt = Math.min(clipLength, Math.max(showFrom, Number(s.hideAt >= 9999 ? clipLength : s.hideAt)));
  return <aside className="settings-panel">
    <div className="panel-heading"><div><p className="section-kicker">04 / 字卡設定</p><h3>編輯字卡</h3></div><span className="editing-label">正在編輯：{layer === 'topText' ? '上欄文字' : '下欄文字'}</span></div>
    <label className="setting-field">片段主題名稱<input value={clip.title || ''} onChange={(e) => update(clip.id, 'title', e.target.value)} /></label>
    <label className="setting-field">輸出比例<select value={clip.outputAspect || 'original'} onChange={(e) => updateAspect(clip.id, e.target.value)}><option value="original">原始比例{dimensions ? `（${dimensions.width} × ${dimensions.height}）` : ''}</option><option value="9:16">9:16 Reels（1080 × 1920）</option><option value="1:1">1:1 正方形（1080 × 1080）</option></select></label>
    {clip.outputAspect !== 'original' && <div className="crop-position"><span className="setting-label">畫面位置（調整裁切範圍）</span><label>X 軸 <output>{Math.round(clip.cropPosition?.x ?? 50)}%</output><input type="range" min="0" max="100" value={clip.cropPosition?.x ?? 50} onChange={(e) => updateCropPosition(clip.id, 'x', e.target.value)} /></label><label>Y 軸 <output>{Math.round(clip.cropPosition?.y ?? 50)}%</output><input type="range" min="0" max="100" value={clip.cropPosition?.y ?? 50} onChange={(e) => updateCropPosition(clip.id, 'y', e.target.value)} /></label><small>拖曳滑桿選擇輸出畫面要保留的區域。</small></div>}
    <label className="setting-field music-field">背景音樂<select value={clip.musicFile || ''} onChange={(e) => update(clip.id, 'musicFile', e.target.value)}><option value="">不使用背景音樂</option>{musicFiles.map((file) => <option key={file} value={file}>{file}</option>)}</select></label>
    {clip.musicFile && <div className="music-controls settings-music-controls"><label>音樂音量 <output>{Math.round(Number(clip.musicVolume ?? 0.35) * 100)}%</output><input type="range" min="0" max="1" step="0.05" value={clip.musicVolume ?? 0.35} onChange={(e) => update(clip.id, 'musicVolume', Number(e.target.value))} /></label><label>原影片音量 <output>{Math.round(Number(clip.originalVolume ?? 1) * 100)}%</output><input type="range" min="0" max="1" step="0.05" value={clip.originalVolume ?? 1} onChange={(e) => update(clip.id, 'originalVolume', Number(e.target.value))} /></label><button type="button" className="music-preview-button" onClick={() => preview(clip)}>{playing ? '暫停試聽' : '試聽此片段'}</button><small>兩個音量會同時套用到試聽與渲染。</small></div>}
    <div className="layer-tabs"><button type="button" className={layer === 'topText' ? 'active' : ''} onClick={() => setLayer(clip.id, 'topText')}>上欄</button><button type="button" className={layer === 'bottomText' ? 'active' : ''} onClick={() => setLayer(clip.id, 'bottomText')}>下欄</button></div>
    <label className="setting-field">文字內容<input value={layer === 'topText' ? clip.top_text : clip.caption} onChange={(e) => update(clip.id, layer === 'topText' ? 'top_text' : 'caption', e.target.value)} /></label>
    <div className="caption-timing"><span className="setting-label">文字出現時間（{layer === 'topText' ? '上欄' : '下欄'}）</span><div><label>出現<input type="number" min="0" max={clipLength} step="0.1" value={s.showFrom === '' ? '' : showFrom} onChange={(e) => updateLayer(clip.id, layer, 'showFrom', e.target.value === '' ? '' : e.target.value)} /></label><label>消失<input type="number" min="0" max={clipLength} step="0.1" value={s.hideAt === '' ? '' : hideAt} onChange={(e) => updateLayer(clip.id, layer, 'hideAt', e.target.value === '' ? '' : e.target.value)} /></label></div><small>以片段開始處為 0 秒，最長 {clipLength.toFixed(1)} 秒</small></div>
    <div className="time-editor"><span className="setting-label">片段時間</span><div><label>開始<input type="number" min="0" step="0.1" value={clip.start_time} onChange={(e) => update(clip.id, 'start_time', e.target.value)} /></label><label>結束<input type="number" min="0" step="0.1" value={clip.end_time} onChange={(e) => update(clip.id, 'end_time', e.target.value)} /></label></div><small>AI 建議 {clip.aiStart.toFixed(1)}s – {clip.aiEnd.toFixed(1)}s</small></div>
    <div className="style-grid"><label className="setting-field">字型<select value={s.fontFamily} onChange={(e) => updateLayer(clip.id, layer, 'fontFamily', e.target.value)}>{fonts.map((font) => <option key={font.value} value={font.value}>{font.label}</option>)}</select></label><label className="setting-field">字級<div className="number-field"><input type="number" min="16" max="240" value={s.fontSize} onChange={(e) => updateLayer(clip.id, layer, 'fontSize', e.target.value)} /></div></label><label className="setting-field color-setting">顏色<div className="color-field"><input type="color" value={s.color} onChange={(e) => updateLayer(clip.id, layer, 'color', e.target.value)} /></div></label></div>
    <div className="position-group"><div className="control-group-title">位置 <button type="button" onClick={() => resetPosition(clip.id, layer)}>重設位置</button></div><label>X 軸 <output>{s.x}%</output><input type="range" min="5" max="95" value={s.x} onChange={(e) => updateLayer(clip.id, layer, 'x', e.target.value)} /></label><label>Y 軸 <output>{s.y}%</output><input type="range" min="5" max="95" value={s.y} onChange={(e) => updateLayer(clip.id, layer, 'y', e.target.value)} /></label></div>
    <div className="settings-actions"><button type="button" className="render-one" onClick={() => renderOne(clip)} disabled={clip.rendering}>{clip.rendering ? '渲染中…' : '渲染此片段 ↗'}</button></div>{clip.renderedUrl && <div className="rendered-result"><video src={clip.renderedUrl} controls preload="metadata" onPlay={(event) => document.querySelectorAll('video').forEach((video) => { if (video !== event.currentTarget) video.pause(); })} /><a href={clip.downloadUrl || clip.renderedUrl}>下載此片段 ↓</a></div>}
  </aside>;
}
